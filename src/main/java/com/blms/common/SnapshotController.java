...
/**
 * 前端联调快照端点（阶段 6 收尾）：
 *   GET /api/snapshot → 34 个集合全量（29 数组型 + 5 对象型）+ logs（op_log 审计，最近 1000，时间倒序）。
...
public class SnapshotController {
    ...
    public SnapshotController(DataStore store, AuditLog audit) {
        this.store = store;
        this.audit = audit;
    }
    ...
    public ApiResult<Map<String, Object>> resetDemo() {
        ...
        Map<String, Object> data = new LinkedHashMap<>();
        data.put("reset", true);
        data.put("note", "内存数据仓库已重置回种子态（仅内存，不回写 DB）");
        return ApiResult.success(data);
    }
}
